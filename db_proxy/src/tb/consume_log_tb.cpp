...
namespace
{
  ...
  public:
    ...
    {
      int db_sid = -1;
      char bf[sizeof(consume_log) + 4];
      stream_istr si(bf, sizeof(bf));

      in_stream is(msg, len);
      is >> db_sid >> si;
      assert(si.str_len() == sizeof(consume_log));

      sql_insert *sql = new sql_insert();
      ::memcpy((char *)&sql->data_, si.str(), sizeof(consume_log));
  ...
  static tb_msg_maper tm_insert(REQ_INSERT_CONSUME_LOG, new tb_msg_insert(RES_INSERT_CONSUME_LOG));
}
