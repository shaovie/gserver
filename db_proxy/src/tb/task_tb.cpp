...
namespace
{
  ...
  public:
    ...
    {
      int db_sid = -1;
      char bf[sizeof(task_info) + 4];
      stream_istr si(bf, sizeof(bf));

      in_stream is(msg, len);
      is >> db_sid >> si;
      assert(si.str_len() == sizeof(task_info));

      sql_insert *sql = new sql_insert();
      ::memcpy((char *)&sql->data_, si.str(), sizeof(task_info));
  ...
  static tb_msg_maper tm_insert(REQ_INSERT_TASK, new tb_msg_insert(RES_INSERT_TASK));
  ...
  class tb_msg_get : public tb_msg_handler
  {
    ...
    public:
      ...
      virtual void read_sql_result(MYSQL_RES *res, async_result *ar)
      {
        ...
        {
          mblock *mb = mblock_pool::instance()->alloc(sizeof(task_info));
          task_info *p = (task_info *)mb->data();
          p->reset();
          this->read_sql_row(p, row);
          mb->wr_ptr(sizeof(task_info));
          ar->push_back(mb);
        }
      }
      void read_sql_row(task_info *data, MYSQL_ROW &row)
      {
        data->char_id_     = ::atoi(row[task_info::IDX_CHAR_ID]);
        data->task_cid_    = ::atoi(row[task_info::IDX_TASK_CID]);
        data->state_       = ::atoi(row[task_info::IDX_STATE]);
        data->value_[0]    = ::atoi(row[task_info::IDX_VALUE_0]);
        data->value_[1]    = ::atoi(row[task_info::IDX_VALUE_1]);
        data->value_[2]    = ::atoi(row[task_info::IDX_VALUE_2]);
        data->extra_value_ = ::atoi(row[task_info::IDX_EXTRA_VALUE]);
        data->accept_time_ = ::atoi(row[task_info::IDX_ACCEPT_TIME]);
      }
    };
  public:
  ...
  static tb_msg_maper tm_get(REQ_GET_TASK_LIST, new tb_msg_get(RES_GET_TASK_LIST));
  ...
  static tb_msg_maper tm_update(REQ_UPDATE_TASK, new tb_msg_update(RES_UPDATE_TASK));
  ...
  public:
    ...
    virtual int handle_msg(proxy_obj *po, const char *msg, const int len)
    {
      ...
      in_stream is(msg, len);
      is >> db_sid >> sql->data_.char_id_ >> sql->data_.task_cid_;
  ...
  static tb_msg_maper tm_delete(REQ_DELETE_TASK, new tb_msg_delete(RES_DELETE_TASK));
}
