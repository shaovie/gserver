...
namespace
{
  ...
  public:
    ...
    {
      int db_sid = -1;
      char bf[sizeof(forbid_opt) + 4];
      stream_istr si(bf, sizeof(bf));

      in_stream is(msg, len);
      is >> db_sid >> si;
      assert(si.str_len() == sizeof(forbid_opt));

      sql_insert *sql = new sql_insert();
      ::memcpy((char *)&sql->data_, si.str(), sizeof(forbid_opt));
  ...
  static tb_msg_maper tm_insert(REQ_INSERT_FORBID_OPT, new tb_msg_insert(RES_INSERT_FORBID_OPT));
}
