...
namespace
{
  ...
  class tb_msg_get : public tb_msg_handler
  {
    ...
    public:
      ...
      void read_sql_row(jing_ji_log *data, MYSQL_ROW &row)
      {
        data->char_id_       = ::atoi(row[jing_ji_log::IDX_CHAR_ID]);
        data->competitor_id_ = ::atoi(row[jing_ji_log::IDX_COMPETITOR_ID]);
        data->rank_change_   = ::atoi(row[jing_ji_log::IDX_RANK_CHANGE]);
        data->time_          = ::atoi(row[jing_ji_log::IDX_TIME]);
      }
      virtual void read_sql_result(MYSQL_RES *res, async_result *ar)
      {
        ...
        {
          mblock *mb = mblock_pool::instance()->alloc(sizeof(jing_ji_log));
          jing_ji_log *ti = (jing_ji_log *)mb->data();
          ti->reset();
          this->read_sql_row(ti, row);
          mb->wr_ptr(sizeof(jing_ji_log));
          ar->push_back(mb);
        }
      }
    };
  ...
  static tb_msg_maper tm_get(REQ_GET_JING_JI_LOG, new tb_msg_get(RES_GET_JING_JI_LOG));
  ...
  public:
    ...
    {
      int db_sid = -1;
      char bf[sizeof(jing_ji_log) + 4];
      stream_istr si(bf, sizeof(bf));

      in_stream is(msg, len);
      is >> db_sid >> si;
      assert(si.str_len() == sizeof(jing_ji_log));

      sql_insert *sql = new sql_insert();
      ::memcpy((char *)&sql->data_, si.str(), sizeof(jing_ji_log));
      db_async_store::instance()->do_sql(po->proxy_id(),
                                         db_sid,
                                         0,
                                         this->resp_id_,
  ...
  static tb_msg_maper tm_insert(REQ_INSERT_JING_JI_LOG, new tb_msg_insert(RES_INSERT_JING_JI_LOG));
}
