...
namespace
{
  ...
  public:
    ...
    {
      int db_sid = -1;
      char bf[sizeof(char_attr) + 4];
      stream_istr si(bf, sizeof(bf));

      in_stream is(msg, len);
      is >> db_sid >> si;
      assert(si.str_len() == sizeof(char_attr));

      sql_update *sql = new sql_update();
      ::memcpy((char *)&sql->data_, si.str(), sizeof(char_attr));
  ...
  static tb_msg_maper tm_update(REQ_UPDATE_CHAR_ATTR, new tb_msg_update(RES_UPDATE_CHAR_ATTR));
}
