...
namespace
{
  ...
  class tb_msg_get : public tb_msg_handler
  {
    ...
    public:
      ...
      virtual void read_sql_result(MYSQL_RES *res, async_result *ar)
      {
        ...
        {
          mblock *mb = mblock_pool::instance()->alloc(sizeof(tianfu_skill_info));
          tianfu_skill_info *p = (tianfu_skill_info *)mb->data();
          p->reset();
          this->read_sql_row(p, row);
          mb->wr_ptr(sizeof(tianfu_skill_info));
          ar->push_back(mb);
        }
      }
      void read_sql_row(tianfu_skill_info *data, MYSQL_ROW &row)
      {
        data->char_id_ = ::atoi(row[tianfu_skill_info::IDX_CHAR_ID]);
        data->cid_     = ::atoi(row[tianfu_skill_info::IDX_CID]);
        data->lvl_     = ::atoi(row[tianfu_skill_info::IDX_LVL]);
        data->exp_     = ::atoi(row[tianfu_skill_info::IDX_EXP]);
      }
    };
  public:
  ...
  static tb_msg_maper tm_tianfu_skill_get_list(REQ_GET_TIANFU_SKILL_LIST, new tb_msg_get(RES_GET_TIANFU_SKILL_LIST));
  ...
  public:
    ...
    {
      int db_sid = -1;
      char bf[sizeof(tianfu_skill_info) + 4];
      stream_istr si(bf, sizeof(bf));

      in_stream is(msg, len);
      is >> db_sid >> si;
      assert(si.str_len() == sizeof(tianfu_skill_info));

      sql_update *sql = new sql_update();
      ::memcpy((char *)&sql->data_, si.str(), sizeof(tianfu_skill_info));
      db_async_store::instance()->do_sql(po->proxy_id(),
                                         db_sid,
                                         0,
                                         this->resp_id_,
  ...
  static tb_msg_maper tm_update(REQ_UPDATE_TIANFU_SKILL, new tb_msg_update(RES_UPDATE_TIANFU_SKILL));
}
