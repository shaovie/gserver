...
namespace
{
  ...
  public:
    ...
    {
      int db_sid = -1;
      char bf[sizeof(worship_info) + 4];
      stream_istr si(bf, sizeof(bf));

      in_stream is(msg, len);
      is >> db_sid >> si;
      assert(si.str_len() == sizeof(worship_info));

      sql_update *sql = new sql_update();
      ::memcpy((char *)&sql->data_, si.str(), sizeof(worship_info));
  ...
  static tb_msg_maper tm_update(REQ_UPDATE_WORSHIP_INFO, new tb_msg_update(RES_UPDATE_WORSHIP_INFO));
